...
pin_config_t PinOfEntry={.port=PORTB_INDEX,.pin=GPIO_PIN0,.direction=GPIO_DIRECTION_INPUT};
pin_config_t PinOfExit={.port=PORTB_INDEX,.pin=GPIO_PIN1,.direction=GPIO_DIRECTION_INPUT};
...
int main(void) {
    ...
    dc_motor_initialize(&DC_Entry);
    dc_motor_initialize(&DC_Exit);
    gpio_Pin_Intialize(&PinOfEntry);
    gpio_Pin_Intialize(&PinOfExit);
    lcd_4bit_send_string_data(&Lcd_Obj,"There Are 0 Cars");
    lcd_4bit_send_string_data_pos(&Lcd_Obj,2,1,"Garage Have Space");
    uint8 u8ReadEntryPin=GPIO_LOW;
    uint8 u8ReadExitPin=GPIO_LOW;
    uint8 u8Counter=0;
    while(1){

        gpio_Pin_Read_Logic(&PinOfEntry,&u8ReadEntryPin);
        if((u8ReadEntryPin==GPIO_HIGH) && (u8Counter < 3) && (u8Counter >= 0)){
            u8Counter++;
            lcd_4bit_send_char_data_pos(&Lcd_Obj,1,11,u8Counter+0x30);
            if(u8Counter==3){
                lcd_4bit_send_command(&Lcd_Obj,0x01);
                lcd_4bit_send_string_data(&Lcd_Obj,"There Are 3 Cars");
                lcd_4bit_send_string_data_pos(&Lcd_Obj,2,1,"Garage Is Full");
            }
            dc_motor_move_right(&DC_Entry);
            _TimerDelay_ms(1000);
            dc_motor_move_left(&DC_Entry);
             _TimerDelay_ms(1000);
             dc_motor_stop(&DC_Entry);
        }
        else { }

        gpio_Pin_Read_Logic(&PinOfExit,&u8ReadExitPin);
        if((u8ReadExitPin==GPIO_HIGH) && (u8Counter > 0) && (u8Counter <=3)){
            u8Counter--;
            lcd_4bit_send_command(&Lcd_Obj,0x01);
            lcd_4bit_send_string_data(&Lcd_Obj,"There Are ");
            lcd_4bit_send_char_data_pos(&Lcd_Obj,1,11,u8Counter+0x30);
            lcd_4bit_send_string_data(&Lcd_Obj," Cars");
            lcd_4bit_send_string_data_pos(&Lcd_Obj,2,1,"Garage Have Space");
            dc_motor_move_right(&DC_Exit);
            _TimerDelay_ms(1000);
            dc_motor_move_left(&DC_Exit);
             _TimerDelay_ms(1000);
             dc_motor_stop(&DC_Exit);
        }
        else { }
